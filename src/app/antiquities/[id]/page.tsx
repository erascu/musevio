...
export default function ItemPage() {
  const [antqsItem, setAntqsItem] = useState<AntqsItem | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState(false);

  const { id } = useParams<{ id: string }>();
  const smallImg = antqsItem?.primaryimageurl
    ? `${antqsItem.primaryimageurl}?width=500`
    : "";
      ...
      {isLoading ? (
        ...
        <div className="max-w-7xl flex flex-col-reverse lg:flex-row lg:gap-16 gap-5 py-8 items-center lg:items-start">
          <div className="lg:w-1/2 self-start text-left">
            <h2 className="playfair !text-3xl font-bold">{antqsItem.title}</h2>
            <h3 className="italic !text-lg text-ring">
              {antqsItem.classification}
            </h3>
            <h3 className="!text-base mt-3 text-ring">
              <span className="text-black">Medium:</span> {antqsItem.medium}
            </h3>
            <h3 className="!text-base text-ring">
              <span className="text-black">Period:</span> {antqsItem.period}
            </h3>
            <h3 className="!text-base text-ring">
